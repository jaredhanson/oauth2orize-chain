 ...
 *
 * This exchange is used to exchange an access token issued by an OAuth domain
 * for a chained access token, potentially issued by a different OAuth domain.
 * This scenario facilitiates service chaining, in which one service
 ...
 * token between separate domains.  Trust must be established between the
 * domains so that the issuer of the original token can be determined, and the
 * token itself can be validated.  Furthermore, there must be a method of
 * mapping the subject in the original domain to a subject in the chained
 * domain.  These mechanisms are not within the scope of this grant type (or
 * this Node package), and must be defined and implemented by the application.
 *
 * References:
...
module.exports = function(options, issue) {
  ...
  if (!issue) { throw new TypeError('chain exchange requires an issue callback'); }
  ...
  return function chain(req, res, next) {
    ...
    try {
      ...
      if (arity == 5) {
        issue(client, scheme, credential, scope, issued);
      } else if (arity == 4) {
